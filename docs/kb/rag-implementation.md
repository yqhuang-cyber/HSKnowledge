...
默认链路里，**浅召回（关键词 + 标签）是正式的第一路策略**，并与轻量向量召回并行，再融合排序。仓库 README 中有面向产品/接入的全链路与浅召回展开说明；下文第 4 节写实现细节。
...
教育类 AI 老师问答里常说的 **「浅召回」**：第一步**不**用复杂深度语义大模型扫库，而是用关键词匹配、知识库索引扫描、简单标签分类，快速捞出「长得像」的候选。我们的 **`keyword` 通道就是浅召回**（`hybrid` 的一路）；另一路是小型 embedding 的轻量语义召回（仍不是对话大模型）。两路候选再融合排序截断 Top-K。
...
```
           ...
           ▼                               ▼
   浅召回：关键词通道                    轻量向量通道
   （字面/子串、标题、拼音等）          （embedding 余弦相似）
   + filters 标签（类型/考纲/级别）      擅长：口语、同义、场景说法
   擅长：专名、词面、语法名
   例：「朋友」精确命中                  例：「点餐」→ 饮食 / 午饭
           │                               │
           └───────────────┬───────────────┘
                           ▼
                    分数归一化后融合排序
...
| `mode` | 行为 | 何时用 |
|--------|------|--------|
| `hybrid`（**默认**） | 浅召回 + 向量融合 | **外挂陪练应用请用这个** |
| `keyword` | 仅浅召回 | 调试专名；或无向量环境 |
| `vector` | 仅语义相似 | 调试口语召回 |

**为什么召回这么关键？**

- 只靠浅召回：学生说「点餐」可能偏到「钟点 / 时量词」（因为都有「点」），陪练参考就偏了。  
- 只靠向量：专名、精确词条有时不够稳。  
- **混合**：专名与口语场景都照顾到——这是本教学 RAG 的核心设计。

可选 `filters`：按 `chunk_types` / `compliance` / `hsk_level` 收窄召回范围（例如只要考纲内词汇+话题）——这也是浅召回里「标签分类」的一部分。

**和「召回 + 精排」两段式的关系：** 我们有「多路捞候选 + 融合排序」，但**没有**独立精排模型（如 cross-encoder）。对当前约 700 块知识规模足够；库变大后再考虑加重精排。
...
> **HSKnowledge 的 RAG = 本地 HSK1 知识块 + 浅召回（关键词/标签）与轻量 BGE 向量召回的 hybrid + 融合排序 HTTP API；外挂 AI 负责把召回结果注入生成链路（如 `ChatRAGText`）。教学场景下，召回质量优先于「硬卡超纲」；检索阶段不用深度对话大模型。**
